...
for (const file of files) {
  ...
  out = out.replace(/<DocsLayout\b([\s\S]*?)>/g, (m, attrs) => {
    ...
    newAttrs = newAttrs.replace(/\btitle\s*=\s*("[^"]*"|'[^']*'|\{[^}]*\})/g, 'seoTitle=$1');
    newAttrs = newAttrs.replace(/\bdescription\s*=\s*("[^"]*"|'[^']*'|\{[^}]*\})/g, 'seoDescription=$1');
    // replace shorthand attribute syntax inside the opening tag
    newAttrs = newAttrs.replace(/\{\s*title\s*\}/g, 'seoTitle={title}');
    newAttrs = newAttrs.replace(/\{\s*description\s*\}/g, 'seoDescription={description}');
